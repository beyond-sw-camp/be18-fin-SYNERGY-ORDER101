...
const searchPlaceholder = computed(() =>
    selectedType.value === 'FRANCHISE' ? '가맹점명 또는 코드 검색...' : '공급업체명 또는 코드 검색...'
)
    ...
    try {
        ...
        if (selectedType.value === 'FRANCHISE') {
            ...
            vendors.value = (data.franchises || []).map(f => ({
                text: f.franchiseName || f.name,
                value: f.franchiseId || f.id,
                code: f.franchiseCode || f.code,
                type: 'FRANCHISE'
            }))
        } else {
            // 공급업체 조회
        ...
        alert(`${selectedType.value === 'FRANCHISE' ? '가맹점' : '공급업체'} 목록을 불러오는 데 실패했습니다.`)
        vendors.value = []
        totalCount.value = 0
    } finally {
        ...
        <div class="modal-container">
            ...
            <section class="modal-body">
                ...
                <div v-else class="vendor-list">
                    ...
                    <div v-for="vendor in filteredVendors" :key="vendor.value" class="vendor-item"
                        :class="{ active: selectedVendor?.value === vendor.value }" @click="selectVendor(vendor)">
                        <div class="vendor-info">
                            <strong>{{ vendor.text }}</strong>
                            <span class="vendor-code">{{ vendor.code || vendor.value }}</span>
                        </div>
                        <svg v-if="selectedVendor?.value === vendor.value" class="check-icon" width="20" height="20"
            ...
            <footer class="modal-footer">
                ...
                <button class="btn-confirm" :disabled="!selectedVendor" @click="confirm">
                    선택 완료
                </button>
            </footer>
        </div>
